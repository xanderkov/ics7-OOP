...
void ConstIterator<T>::_checkIndex(const string hint) const 
{
    ...
    throw IteratorIndexError(asctime(local_time), __FILE__, typeid(*this).name(), __LINE__, hint);
}
...
void ConstIterator<T>::_checkValid(const string hint) const 
{
    if (!isValid()) 
    {
        time_t cur_time = time(NULL);
        auto local_time = localtime(&cur_time);
        throw IteratorValidationError(asctime(local_time), __FILE__, typeid(*this).name(), __LINE__, hint);
    }
}
